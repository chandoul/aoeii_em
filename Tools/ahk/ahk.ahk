...
If FileExist(ahkapp.hotkeys) {
    ...
    If !FileExist(ahkapp.ahkLocation '\' hashSum '.ahk') {
        FileAppend(script, ahkapp.ahkLocation '\' hashSum '.ahk')
    }
    Try Run(ahkapp.ahkLocation '\' hashSum '.ahk ' ProcessExist())
}
...
readHotkey() => FileExist(ahkapp.hotkeys) ? JSON.LoadFile(ahkapp.hotkeys) : Map()